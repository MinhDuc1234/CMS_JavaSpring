...
public class StringUtil {
    ...
    public static String toQueryString(String input) {
        String query = VietnameseString.removeAccent(input.toLowerCase()).replaceAll("[^a-z0-9\\s]+", "").replaceAll("\\s{2,}", " ").trim();
        return query.substring(0, Math.min(100, query.length()));
    }
